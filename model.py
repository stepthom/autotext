...
class BaseEstimator:
    ...
    def __init__(self, task='binary:logistic', **params):
        ...
        self._task = task
        self.enc = ce.wrapper.PolynomialWrapper(ce.target_encoder.TargetEncoder(handle_unknown="value", min_samples_leaf=10, smoothing=0.1))
    ...
    def _fit(self, X_train, y_train, **kwargs):
        ...
        X_train = self._preprocess(X_train, y_train, fit=True)
    ...
    def predict(self, X_test):
        ...
        if self._model is not None:
            X_test = self._preprocess(X_test, y=None, fit=False)
...
class LGBMEstimator(BaseEstimator):
    ...
    def __init__(self, task='binary:logistic', log_max_bin=8, **params):
        ...
        self._train_size = 0
        #self.enc = ce.wrapper.PolynomialWrapper(ce.james_stein.JamesSteinEncoder(handle_unknown="value", randomized=True))
        #self.enc = ce.wrapper.PolynomialWrapper(ce.target_encoder.TargetEncoder(handle_unknown="value", min_samples_leaf=10, smoothing=0.1))

    def _preprocess(self, X, y=None, fit=False):

        #print("Ina LGBMEstimator::_preprocess. fit={}, X.shape={}, y={}".format(fit, X.shape, None if y is None else y.shape), file=sys.stderr)
        #print('cat cols: {}'.format(X.select_dtypes(include=['object', 'category']).columns))
...
class XGBoostEstimator(SKLearnEstimator):
    ...
    def fit(self, X_train, y_train, budget=None, **kwargs):
        ...
        if not issparse(X_train):
            ...
            X_train = self._preprocess(X_train, fit=True)
    ...
    def predict(self, X_test):
        if not issparse(X_test):
            X_test = self._preprocess(X_test, fit=False)
